        ...
        <div className="max-w-4xl mx-auto mt-5 xl:ml-[263px] ml-[60px]">
          <div className="bg-white p-6 rounded-xl shadow-lg mb-6  w-[90%] ml-[30px]">
            ...
            <div className="flex-grow flex flex-row justify-between items-center">
                <h2 className="text-xl font-semibold mb-2">Create a New Community</h2>
                <button className="bg-custom-color text-white px-3 py-1 rounded-lg ml-auto text-md">Let's Go</button>
                </div>

            </div>
            <p className="text-md mb-2 text-[#11181C]">Create a new community for collaborative learning and unlock the power of shared knowledge and mutual growth</p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 ml-[40px] mr-[50px]">
            {communities.map((community, index) => (
              <div key={index} className="bg-white p-4 rounded-lg shadow-md flex flex-col ">
                ...
                <p className="text-gray-600 mb-2 text-md">{community.description}</p>
                <div className="text-gray-400 text-sm mb-4"> <i className="fas fa-users mr-2"></i>{community.members}</div>
