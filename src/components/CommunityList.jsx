...
const CommunityList = () => {
  ...
  const deleteCommunity = (id) => {
    axios
      .delete(`http://localhost:8090/api/communities/${id}`)
      .then(() => {
    ...
    <div className="flex">
      ...
      <div className="container ml-[300px] mt-10 mr-12">
        ...
        <div className="flex justify-between mt-4">
          <div>
            Showing {currentCommunities.length} of {filteredCommunities.length}
          </div>
          <div className="flex space-x-1">
            {Array.from({ length: Math.ceil(filteredCommunities.length / communitiesPerPage) }).map(
              (_, index) => (
                <button
                  key={index}
                  className={`px-2 py-1 rounded-md ${
                    currentPage === index + 1 ? 'bg-[#D4944C]' : 'bg-gray-200'
                  }`}
        ...
        {isPopupOpen && selectedCommunity && (
          ...
          >
            ...
            >
              ...
              <p className="mb-4">
                <strong>Community ID:</strong> {selectedCommunity.id}
              </p>
              <p className="mb-4">
                <strong>Description:</strong> {selectedCommunity.description}
              </p>
              <p className="mb-4">
                <strong>Created:</strong> {selectedCommunity.created}
              </p>
              <p className="mb-4">
                <strong>Member Count:</strong> {selectedCommunity.memberCount}
              </p>
            </div>
          </div>
        )}
