...
// import AdminSidebar from './AdminSidebar';
// import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
// import { faEye, faTrash, faLock, faUnlock } from '@fortawesome/free-solid-svg-icons';
// import axios from 'axios';
...
//       alert('User and their profile deleted successfully');
//     } catch (error) {
//       console.error('Error deleting user:', error);
//       alert('Failed to delete user and profile');
//     }
//   };
...
// const fetchBlockCount = async (userId) => {
//   try {
//     const response = await axios.get(`http://localhost:8080/api/v1/users/${userId}/blockCount`);
...
//   try {
//     const response = await axios.get(`http://localhost:8080/api/profiles/${userId}`);
//     const blockCount = await fetchBlockCount(userId); // Fetch block count
//     setSelectedProfile({ ...response.data, blockCount }); // Include block count in profile
//     setShowModal(true); // Show modal with the profile
//   } catch (error) {
//     console.error('Error fetching profile details:', error);
//     alert('Failed to load user details');
...
// //   try {
// //     // Send the block status as a JSON body
// //     const response = await axios.put(
// //       `http://localhost:8080/api/v1/users/${userId}/block`,
// //       { blocked }
// //     );
// //     setUsers(users.map(user => 
// //       user.id === userId ? { ...user, blocked } : user
// //     ));
// //     alert(blocked ? 'User has been blocked' : 'User has been unblocked');
// //   } catch (error) {
...
// //     const response = await axios.put(
// //       `http://localhost:8080/api/v1/users/${userId}/block`,
// //       { blocked: !blocked }  // Toggle the blocked status
// //     );

// //     // Update the users list or show message based on response
// //     setUsers(users.map(user => 
// //       user.id === userId ? { ...user, blocked: !blocked } : user
// //     ));

// //     alert(blocked ? 'User has been unblocked' : 'User has been blocked');
// //   } catch (error) {
// //     console.error('Error blocking user:', error);
// //     alert('Failed to block user');
// //   }
...
//     // Update the users list with new block status and count
//     setUsers(users.map(user => 
//       user.id === userId 
//         ? { ...user, blocked: !blocked, blockCount: !blocked ? user.blockCount + 1 : user.blockCount }
//         : user
//     ));
...
// //     )); // Update the state with the new blocked status
...
//               <p className="py-2"><strong>Phone Number:</strong> {selectedProfile.phoneNumber || 'N/A'}</p>
//               <p className="py-2" ><strong>Emergency Contact:</strong> {selectedProfile.emergencyContact || 'N/A'}</p>
//               <p className="py-2"><strong>Block Count:</strong> {selectedProfile.blockCount}</p>
            ...
            {currentUsers.map((user) => (
              <tr key={user.id} className="border-b">
                ...
                <td className="py-2 flex items-center space-x-2 text-[#8B909A]">
                  ...
                  <button onClick={() => handleBlockUser(user.id, user.blocked)}>
                    <FontAwesomeIcon icon={user.blocked ?  faLock : faUnlock} />
                  </button>
                </td>
              </tr>
            ))}
