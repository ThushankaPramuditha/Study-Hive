...
import React, { useState, useEffect } from "react";
import axios from "axios";
import SideBarnNavbar from "./SideBarnNavbar"; // Assuming this is a component
...
const UserInquiries = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:8090/api/inquiries/user", {
        headers: {
          Authorization: `Bearer ${token}`, // Fix: Added backticks for template literal
        },
      })
      .then((response) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:8090/api/inquiries/create", // API endpoint to create an inquiry
        newInquiry,
        {
          headers: {
            Authorization: `Bearer ${token}`, // Fix: Added backticks for template literal
